...
export default function Settings() {
  ...
  return (
    <>
      ...
      <SettingsInner
        settings={value?.linkedSources?.[0].project}
...
type FormState = Partial<ProjectSettings> & {
...
const SettingsInner = ({
  ...
  const handleSubmit = async () => {
    if (
      !formState.apiKey ||
      !formState.apiUsername ||
      !formState.projectId ||
      !formState.projectUrl
    ) {
      setError(
        new Error(
          "Be sure to provide a Project Id, Project URL, API Username, and API Key"
        )
      );
      return;
    }
